  ...
  import { getFirestore, collection, addDoc, query, onSnapshot, orderBy , where } from "firebase/firestore";
  ...
  function ChatTriggerUI() {
    ...
    const webhookURL = "https://podhealthn8n.4gd.ai/prod/v1/e1e0ee50-1d9f-41c1-8910-0d383bb8ddeb/chat";
  ...
  try {
    const res = await fetch(webhookURL, {
      ...
      body: JSON.stringify({ chatInput: message, sessionId: "unique-session-id" }),
    });
    ...
    const isTable = data.output && data.output.startsWith("|");
    const newResponse = { type: "bot", text: data.output || "No response from the agent", isTable };
    ...
    setConversation((prev) => [...prev, newMessage, { type: "bot", text: `Error: ${error.message}` }]);
  } finally {
    setLoading(false);
  }
};
